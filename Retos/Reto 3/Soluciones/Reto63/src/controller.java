...
public class controller {
    ...
    void clickProcessButton(ActionEvent event) {
        ...
        //nombre y nivel de riesgo de cada cuerpo de agua        
        String nombre_riesgo;
        for (int i = 0; i < cuerposDeAguasS.length; i++) {
            nombre_riesgo = datos[i].getNombre()+" "+datos[i].nivel();
            System.out.println(nombre_riesgo);
            richTextField2.setText(richTextField2.getText() + nombre_riesgo +"\n");
        }      
          
        
        //Numero de cuerpos de agua con irca mayor a 14
        double cont = 0;
        for (int i = 0; i < cuerposDeAguasS.length; i++) {
