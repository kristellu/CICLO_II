...
public class Reto87 {
    ...
    public static void printAlertsMode(int[] counts) {
        ...
        for (int i = 0; i < 5; i++) {
            if (counts[i] == max) mode = mode.concat(alerts[i] + ";");
        }
        String[] modeArr = mode.split(";");
        if (modeArr.length == 5) System.out.println("no hay moda");
        else {
    ...
    public static void printTreatmentsMode(int[] counts) {
        ...
        for (int i = 0; i < 3; i++) {
            if (counts[i] == max) mode = mode.concat(treatments[i] + ";");
        }
        String[] modeArr = mode.split(";");
        if (modeArr.length == 3) System.out.println("no hay moda");
        else {
            for (String s : modeArr) System.out.println(s);
        }
    }
