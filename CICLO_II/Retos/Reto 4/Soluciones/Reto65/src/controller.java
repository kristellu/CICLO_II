...
public class controller {
    ...
    void processDataButtonClick(ActionEvent event) {
		...
		//nivel de riesgo e identificador de cada cuerpo de agua        
        String nivel_id;
        for (int i = 0; i < cuerposDeAguasS.length; i++) {
            nivel_id = datos[i].nivel()+" "+String.format("%.2f", datos[i].getId());
            System.out.println(nivel_id);
            TextArea2.setText(TextArea2.getText() + nivel_id +"\n");
        }

        //Numero de cuerpos de agua con irca menor a 50
        double cont = 0;
        for (int i = 0; i < cuerposDeAguasS.length; i++) {
        ...
        //Nombre de cuerpos de agua con nivel de riesgo entre SIN RIESGO y BAJO
        double contm = 0;
        for (int i = 0; i < cuerposDeAguasS.length; i++) {
            if (datos[i].getIrca() > 0 && datos[i].getIrca() <= 14) {
