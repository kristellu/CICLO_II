...
public class controller {
    ...
    void processDataButtonClick(ActionEvent event) {
        ...
        for (int i = 0; i < cuerposDeAguasS.length; i++) {
            ...
            TextArea2.setText(TextArea2.getText() + nombre_irca +"\n");
        }
        ...
        //Nombre de los municipios con cuerpos de agua con nivel de riesgo BAJO
        double contm = 0;
        for (int i = 0; i < cuerposDeAguasS.length; i++) {
            if (datos[i].getIrca() > 5 && datos[i].getIrca() <= 14) {
                System.out.print(datos[i].getMunicipio()+ " ");
                TextArea2.setText(TextArea2.getText() + datos[i].getMunicipio() + " ");
                contm++;
            }
        }
        if (contm == 0) {
