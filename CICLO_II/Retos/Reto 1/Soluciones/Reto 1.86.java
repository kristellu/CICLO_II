...
public class Reto86 {
    ...
    public static float[] getVarAndSum(float[] arr) {
        float tot = 0;
        ...
        float totVar = 0;
        for (float elem : arr) {
            totVar += (elem - result[0]) * (elem - result[0]);
        }
        result[0] = (float) totVar/(arr.length - 1);
        result[1] = tot;
        return result;
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        float[] varAndSum = new float[2];
        varAndSum = getVarAndSum(noduleSizes);
        String varNoduleSize = String.format("%.2f", varAndSum[0]);
        float sumNoduleSize = varAndSum[1];
        System.out.println(varNoduleSize);
        System.out.println(sumNoduleSize);
    }
}