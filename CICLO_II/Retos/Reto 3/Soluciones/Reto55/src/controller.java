...
public class controller {
    ...
    void clickProcessButton(ActionEvent event) {
        ...
        for (int i = 0; i < cuerposDeAguasS.length; i++) {
            ...
            richTextField2.setText(richTextField2.getText() + String.format("%.2f", id) +"\n");
        }

        // Numero de cuerpos de agua con nivel de riesgo entre SIN RIESGO y MEDIO
        double cont = 0;
        for (int i = 0; i < cuerposDeAguasS.length; i++) {
            if (datos[i].getIrca() > 0 && datos[i].getIrca() <= 35) {
                cont++;
            }
